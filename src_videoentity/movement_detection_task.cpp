...
struct MotionDescription
{
    ...
    MotionDescription() = default;
    MotionDescription(MotionDescription&& ) = default;
    MotionDescription(const Type &type, const State &state) : type(type), state(state)
    { }

    Type type = Type::Uncertain;
...
struct TimeSegment
{
    ...
    TimestampOptional begin;

    TimeSegment() = default;

    /*Set to current time.*/
    void time() { begin = std::chrono::system_clock::now(); }

    /** @return TRUE if is not in 0-state(Epoch begin)*/
    bool is_timed() const { return begin.operator bool(); }

    /** @return TRUE if now is greater than (begin + duration)*/
    bool is_elapsed(const Timestamp& now) const
    {
        return is_timed() && (now - begin.value()) > duration;
    }

    /** reset to the beginning of the Epoch */
    void clear()
    {
        begin = TimestampOptional();
        duration = Timespan(0);
    }
};
...
struct TresholdDelayedTrigger
{
    ...
    State input(int tristate/*{-1, 0, 1}*/)
    {
        ...
        {   //case we have level change
            ...
            if (delay.is_elapsed(std::chrono::system_clock::now()))
    ...
    void set_delay(const Timespan& delay_usec)
    { delay.duration = delay_usec;}
...
struct MotionDelayedTrigger
{
    ...
    {
        blick_trigger.set_delay(Timespan(100 * 1000/*usec*/));
        object_motion_trigger.set_delay(Timespan(500 * 1000/*usec*/));
    }
    ...
    void set_blick_treshold_delay(const std::int64_t& msec) {
        blick_trigger.set_delay(Timespan(msec * 1000));
    }

    void set_object_motion_time(const std::int64_t& msec) {
        object_motion_trigger.set_delay(Timespan(msec * 1000));;
    }
...
struct BGSParams
{
    ...
    bool with_downscale = true;
    int skip = 40;
    int frame_cnt = 48;

    double threshold = 0.15;
    double blur_sigma = 2.0;
    double deviation_ratio = 0.005;
};
...
public:
    ...
    MotionDescription&& proc(ZMB::PictureHolder&& frame)
    {
        MotionDescription res;
        res.inputFrameCargo = std::move(frame);

        auto fnImgThreshold = [this, &res](ZMB::PictureHolder& _pic)
        {
            ...
            std::int64_t nonzero = cv::countNonZero(thresholded);
            std::int64_t level = params.deviation_ratio * sz.height() * sz.width();

            MotionDescription trackedMotion = std::move(frame_treshold_track.track(fn_level_tristate(nonzero, level)));
            res.state = trackedMotion.state;
            res.type = trackedMotion.type;
            return;
        };
    ...
    inline int fn_level_tristate(const std::int64_t& value, const std::int64_t& level)
    {
...
public:
    ...
    bool add_rectangular_enabled_zones(const ZMB::MRegion* rectangles_vector, int len)
    {
        ...
        for (auto iter = rectangles_vector; iter != rvend; ++iter)
        {
            ...
            std::unique_ptr<CVBGS::MOG2Algo> mg(new CVBGS::MOG2Algo);
            if (r.square() < 128 * 128)
            {// disable downscaling for little blocks
                mg->params.with_downscale = false;
            }
            rect_zones_map[r] = std::move(mg);
        }
    ...
    CVBGS::MotionDescription&& detect(ZMB::PictureHolder&& frame)
    {
        if (need_mask_update)
        {
...
//            desc = std::move(full_frame_mog2->proc(std::move(frame)));
//            break;
...
MovementDetectionTask::MovementDetectionTask(const std::string &taskName)
    : name(taskName)
{
