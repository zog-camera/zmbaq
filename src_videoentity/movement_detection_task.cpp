...
namespace CVBGS
{
  ...
  typedef std::chrone::microseconds Timespan;
  typedef std::chrono::system_clock::time_point Timespamp;
   ...
   */
  struct TimeSegment : public std::pair<Timespan/*segment legth*/,
					Timestamp /*current time or Epoch*/>
  {
    TimeSegment()
    {
      clear();
    ...
    TimeSegment(Timespan duration) : TimeSegment()
    { set_time_segment(duration); }

    void set_time_segment(Timespan duration)
    { first = duration; }

    inline const Timestamp& ts() const {return second;}
    inline const Timespan& duration() const {return first;}
    ...
    bool is_timed() const
    {
      return second > Timestamp::TIMEVAL_MIN;
    }
...
namespace ZMBEntities {
  ...
  public:
    ...
    bool add_rectangular_enabled_zones(const ZMB::MRegion* rectangles_vector, int len)
    {
	  ...
	  std::shared_ptr<CVBGS::MOG2Algo> mg(new CVBGS::MOG2Algo);
	  if (r.square() < 128 * 128)
