...
struct FFileWriterPV
{
 ...
 explicit  PV(FFileWriter* parent, const AVFormatContext *av_in_fmt_ctx, const std::string& dst);
};

FFileWriterPV::FFileWriterPV(FFileWriter* parent, const AVFormatContext *av_in_fmt_ctx, const std::string& dst)
  : d_parent(parent)
...
FFileWriterPV::~FFileWriterPV()
{
  close();
}

bool FFileWriterPV::open()
{
  ...
  if (!outFmtContext)
    {
      d_parent->onError("Could not create output context");
      return false;
    }
  ...
  for (uint32_t i = 0; i < inFmtContext->nb_streams; ++i)
      ...
      if (inFmtContext->streams[i]->codecpar->codec_type == AVMEDIA_TYPE_VIDEO)
	{
	  ...
	  if (!out_stream)
	    {
	      d_parent->onError("Failed allocating output stream");
	      goto _on_fail_;
	    }
...
void FFileWriterPV::write(std::shared_ptr<av::Packet> input_avpacket)
{
